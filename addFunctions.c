...
/**
 * custom_erratoi - Converts a string to an integer with error handling.
 * @s: The string to be converted.
...
void custom_print_error(info_t *info, char *estr)
{
	_eputs(info->fname);
	_eputs(": ");
	custom_print_decimal(info->line_count, STDERR_FILENO);
	_eputs(": ");
	_eputs(info->argv[0]);
	_eputs(": ");
	_eputs(estr);
...
/**
 * custom_print_decimal - Prints a decimal (integer) number (base 10).
 * @input: The input.
 * @fd: The file descriptor to write to.
 * Return: Number of characters printed.
 */
int custom_print_decimal(int input, int fd)
{
